...
def fetch_chat(api_key, version, continuation, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"⚠️ {type(e).__name__}: {e} — 正在重试 {attempt+1}/{retries}")
            time.sleep(3)
    raise RuntimeError("❌ 多次重试后仍无法获取数据。")
...
def main(url):
    ...
    if not yid:
        print("❌ 找不到 ytInitialData。可能需要 Cookie。")
        return

    continuation = find_continuation(yid)
    if not continuation:
        print("❌ 找不到 continuation。")
        return
    ...
    for i in range(3000):
        ...
        if max_seen_offset / 1000 >= duration:
            print(f"🏁 已到达视频时长（{duration}秒），流程结束。")
            break
        ...
        if not next_c:
            print("🟢 continuation 已耗尽，流程结束。")
            break
        continuation = next_c

        if i % 20 == 0:
            elapsed = int(time.time() - start_time)
            print(f"⏳ 已耗时 {elapsed}秒 / 已获取 {total}条 / 当前进度 {max_seen_offset//1000}秒")
    ...
    try:
        ...
        if removed > 0:
            print(f"🧽 已删除 {removed} 行重复数据。")
    except Exception as e:
        print(f"⚠️ 删除重复数据时出错：{e}")
